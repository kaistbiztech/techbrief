...
## 3. 후보 풀 구성 (목표: 최소 40개, 권장 50개)

큐레이션 전에 먼저 `Message/<id>/curation-log.md`의 "후보 풀" 섹션에 원래 모은 후보를 모두 남긴다. 최종 10개만 적지 않는다. 직접 접근한 매체와 검색 쿼리에서 발견한 후보를 합쳐 **최소 40개**를 목표로 하고, 30개 미만이면 한국 매체 1회·글로벌 매체 1회·아티클/뉴스레터 1회를 추가 검색한다.

각 후보는 다음 필드를 빠짐없이 메모한다:

```
- 발견 채널: 직접 접근 매체명 또는 검색 쿼리
- 원문 헤드라인
- 사건 한 줄 요약
...
- 핵심 숫자/돈/사용자/거래 규모
- 1차 URL
- 보조 URL 또는 한국어 보도 URL (있으면)
- 카테고리 (A/B/C/D) + 산업 분야
- MBA 테크 네트워크 관점의 한 줄 가치: "왜 이 사람들이 클릭하거나 단톡방에서 물어볼 만한가"
- 상태: 평가 전 / 중복 탈락 / 날짜 탈락 / 점수 탈락 / 다양성 보류 / 최종 선정
```
...
후보 풀에는 탈락 후보도 남긴다. 나중에 "왜 이 10개인가"를 설명하려면 탈락한 좋은 후보와 그 이유가 같이 보여야 한다.
